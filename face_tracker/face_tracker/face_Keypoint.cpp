...
int
face_tracker::
track(const Mat &im,Rect face_rect,const face_tracker_params &p)
{
  ...
  if(!tracking)
  {
    //points = detector.detect(gray,p.scaleFactor,p.minNeighbours,p.minSize);
	  points = detector.detect_Rect(gray,p.scaleFactor,p.minNeighbours,p.minSize,face_rect);
     ...
     const Scalar con_color)
{
  ...
  for(int i = 0; i < smodel.C.rows; i++){
   ...
   // line(im,points[j],points[k],con_color,1);
	 //circle(im,points[j],1,con_color);
  }
  /*line(im,points[41],points[31],con_color,2);
  line(im,points[41],points[36],con_color,2);
  line(im,points[31],points[36],con_color,2);*/
   circle(im,points[31],2,pts_color,2,CV_AA);
   circle(im,points[36],2,pts_color,2,CV_AA);
  /*for(int i = 15; i < n; i++)
	 circle(im,points[i],1,pts_color,2,CV_AA);*/
  //¼ÆËã¼Ð½Ç
  return 0;
}
//==============================================================================
